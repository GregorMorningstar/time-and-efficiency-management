...
interface PageProps {
  // jeśli w propsach masz inne dane – dopisz tutaj
  [key: string]: any;
}
...
const baseItems: NavItem[] = [
  ...
  { label: 'Awarie', href: '/failures' },
  { label: 'Support', href: '/support' },
];

export default function EmployeeTopNav() {
  const [open, setOpen] = useState(false);
  // Inertia 1.x: usePage().url może być niedostępne typowo – fallback do window.location
  ...
  return (
    <nav
      className="w-full bg-slate-900 text-white shadow-md border-b border-slate-800"
      role="navigation"
          ...
          <ul className="flex flex-col gap-2 mt-4 mb-2 lg:my-0 lg:flex-row lg:items-center lg:gap-6">
            {items.map((item) => {
              ...
              return (
                <li
                  key={item.href}
                  className={`flex items-center p-1 text-sm gap-x-2 transition-colors ${
                    active ? 'text-white font-medium' : 'text-gray-300'
                  }`}
                >
                  <Link
                    href={item.href}
                    className="flex items-center hover:text-white focus:outline-none focus:ring-2 focus:ring-emerald-400 rounded-sm"
